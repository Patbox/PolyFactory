...
public class GuiUtils {
    ...
    public static void drawFlatNumbers(SlotBasedGui gui, int position, int value, int size, int color, boolean leadingZero) {
        drawNumberInternal(gui, position, value, size, color, leadingZero, GuiTextures.NUMBERS_FLAT_24);
    }

    //public static void drawShadowNumbers(SlotBasedGui gui, int position, int value, int size, int color, boolean leadingZero) {
    //    drawNumberInternal(gui, position, value, size, color, leadingZero, GuiTextures.NUMBERS_SHADOW);
    //}

    private static void drawNumberInternal(SlotBasedGui gui, int position, int value, int size, int color, boolean leadingZero, IntFunction<GuiElementBuilder>[] numbers) {
    ...
    public static GuiElement createDynamicButton(Supplier<ItemStack> itemSupplier) {
        return createDynamicButton(itemSupplier, GuiElement.EMPTY_CALLBACK);
    }

    public static GuiElement createDynamicButton(Supplier<ItemStack> itemSupplier, Consumer<ClickType> clickTypeConsumer) {
        return createDynamicButton(itemSupplier, (index, type, action, gui) -> clickTypeConsumer.accept(type));
    }

    public static GuiElement createDynamicButton(Supplier<ItemStack> itemSupplier, GuiElement.ClickCallback callback) {
        return new GuiElement() {
            @Override
            public ItemStack getItemStack() {
