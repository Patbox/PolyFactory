...
import eu.pb4.polyfactory.mixin.fabricbkp.BundleContentsComponentAccessor;
...
public class BundleContentsStorage implements Storage<ItemVariant> {
	...
	private boolean updateStack(ComponentChanges changes, TransactionContext transaction) {
		ItemVariant newVariant = PFItemStorage.withComponentChanges(ctx.getItemVariant(), changes);
	...
	BundleContentsComponent bundleContents() {
		return ctx.getItemVariant().toStack().getComponents().getOrDefault(DataComponentTypes.BUNDLE_CONTENTS, BundleContentsComponent.DEFAULT);
	}
