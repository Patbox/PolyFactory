    ...
    protected void onHitEntity(EntityHitResult entityHitResult) {
        if (this.random.nextFloat() < 0.3) {
            ...
            if (level() instanceof ServerLevel && entityHitResult.getEntity() instanceof LivingEntity livingEntity) {
                ...
                var effect = livingEntity.getEffect(FactoryEffects.STICKY_SLIME);
                int time = 20;
                if (effect != null) {
                    time += effect.getDuration();
                }
                livingEntity.addEffect(new MobEffectInstance(FactoryEffects.STICKY_SLIME, Math.min(time, 20 * 60), 0), this);
            }
        }
