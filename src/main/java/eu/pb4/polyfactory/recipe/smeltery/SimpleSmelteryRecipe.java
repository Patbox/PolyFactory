...
public record SimpleSmelteryRecipe(String group, Ingredient ingredient, List<FluidStack<?>> output, int time) implements SmelteryRecipe {
    ...
    public static RecipeHolder<SimpleSmelteryRecipe> of(String group, HolderSet.Named<Item> ingredient, FluidStack<?> output, int time) {
        return new RecipeHolder<>(FactoryUtil.recipeKey("smeltery/from_" + ingredient.key().location().getPath()), new SimpleSmelteryRecipe(group, Ingredient.of(ingredient), List.of(output), time));
    }

    public static RecipeHolder<SimpleSmelteryRecipe> of(HolderSet.Named<Item> ingredient, FluidStack<?> output, int time) {
        return new RecipeHolder<>(FactoryUtil.recipeKey("smeltery/from_" + ingredient.key().location().getPath()), new SimpleSmelteryRecipe("",  Ingredient.of(ingredient), List.of(output), time));
    }
