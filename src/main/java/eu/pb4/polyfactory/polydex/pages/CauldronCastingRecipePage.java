...
public abstract class CauldronCastingRecipePage<T extends CauldronCastingRecipe> extends PrioritizedRecipePage<T> {
    private static final ItemStack ICON = new GuiElementBuilder(id("-/block/casting_cauldron")).setName(Text.translatable("block.polyfactory.casting_cauldron")).asStack();
    private final List<PolydexIngredient<?>> ingredients;
    private final PolydexStack<?> output;

    public CauldronCastingRecipePage(RecipeEntry<T> recipe) {
        super(recipe);
        this.ingredients = PolydexCompatImpl.createIngredientsReg(List.of(), List.of(getBaseFluid()));
